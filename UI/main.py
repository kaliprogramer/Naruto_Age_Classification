...
# ⚠️ Update this with your actual labels
class_names = ["adult", "teen", "young"]  # placeholder, adjust if 10 classes
...
@st.cache_resource
def load_model():
    model = SimpleCNN(num_classes=10, img_size=IMG_SIZE)
    # compute an absolute path for weights based on this file's location
    base_dir = os.path.dirname(os.path.abspath(__file__))
    weights_path = os.path.join(base_dir, "..", "Core", "naruto_cnn_weights.pth")
    try:
        ...
        st.error(f"Model weights not found! Checked path: {weights_path}")
        return None
    except RuntimeError as e:
        st.error(f"Model architecture mismatch: {e}")
        st.info("Your saved model expects 10 output classes. Either:\n"
                "- Retrain with 3 classes, or\n"
                "- Update `class_names` to match 10 labels.")
        return None
...
st.markdown("""
    ...
    .main-header { font-family: 'Orbitron', sans-serif; text-align: center; color: #FFD700; text-shadow: 0 0 10px #FF4500, 2px 2px 8px black; font-size: 3.5rem; margin-bottom: 0; }
    .sub-header { font-family: 'Orbitron', sans-serif; text-align: center; color: #FFA500; font-size: 1.8rem; background: rgba(0,0,0,0.5); border-radius: 50px; padding: 10px; width: 70%; margin: 20px auto; backdrop-filter: blur(5px); }
    .prediction-card { padding: 20px; border-radius: 20px; background: rgba(30,30,30,0.8); backdrop-filter: blur(8px); border: 2px solid #FF8C00; box-shadow: 0 0 20px #FF4500; text-align: center; }
    .footer { text-align: center; color: #FFD700; font-weight: bold; margin-top: 50px; background: linear-gradient(90deg, transparent, #FF4500, transparent); padding: 10px; border-radius: 10px; }
    .stButton>button { background: linear-gradient(45deg, #FF8C00, #FF4500); color: white; font-weight: bold; border: none; border-radius: 50px; padding: 10px 30px; box-shadow: 0 4px 15px rgba(255,69,0,0.4); transition: 0.3s; }
    .stButton>button:hover { transform: scale(1.05); box-shadow: 0 6px 20px rgba(255,69,0,0.6); }
    .stFileUploader { background: rgba(0,0,0,0.5); border-radius: 20px; padding: 20px; backdrop-filter: blur(5px); }
    </style>
...
with col1:
    ...
    uploaded_file = st.file_uploader("Upload an image", type=["jpg","jpeg","png","webp"], label_visibility="collapsed")
    ...
    if img is not None and model is not None:
        with st.spinner("Analyzing chakra signature..."):
            ...
            for i in range(len(class_names)):
                st.markdown(f"**{i}: {class_names[i]}:** {probs[i]*100:.2f}%")
    else:
